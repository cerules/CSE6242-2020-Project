...
function drawChart() {
  ...
  function tickActions() {
    if (iterations % 100 === 0 && !document.getElementById("loadingdiv")) {
      var loadingDiv = d3.select("body").append("div").attr("id", "loadingdiv");
...
d3.csv("../data/ontologyGraph/words.csv", function (data) {
  ...
  d3.csv("../data/ontologyGraph/edges.csv", function (data) {
    data.forEach(function (d) {
      var skipFlag = 1;
      for (i = 0; i < links.length; i++) {
        var thisLink = links[i];
        if (
          thisLink.source == idNodeMap[d.target] &&
          thisLink.target == idNodeMap[d.source]
        ) {
          skipFlag = 0;
          break;
        }
      ...
      if (skipFlag) {
        var tempdata = {
          source: idNodeMap[d.source],
          target: idNodeMap[d.target],
    ...
    links.forEach(function (thisLink) {
      thisLink.source =
        nodes[thisLink.source] ||
        (nodes[thisLink.source] = { name: thisLink.source });
      thisLink.target =
        nodes[thisLink.target] ||
    ...
    var autocompDiv = autocompForm
      .append("div")
      .attr("class", "autocomplete")
      .attr("style", "width:275px;margin:2px");

    var autocompEntry = autocompDiv
      .append("input")
      .attr("id", "myInput")
      .attr("type", "text")
    ...
    function deleteEdge() {
      ...
      for (let i = links.length - 1; i >= 0; i--) {
        if (
          links[i].source.name.toLowerCase() === source &&
          links[i].target.name.toLowerCase() === target
        ) {
          links.splice(i, 1);
        } else if (
          links[i].source.name.toLowerCase() === target &&
    ...
    container = d3.select("body").append("div").attr("id", "halfpage");
    drawChart(container);
    ...
    footer
      ...
      .text(
        "CSE 6242(Spring2020) -- Team 189 -- (Prashanthi Narayanamangalam, Christopher Roth, Jonathan Storey)"
      );
  });
});
